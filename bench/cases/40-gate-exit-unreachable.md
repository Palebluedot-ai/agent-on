...
| 清空脏文件 / 删 worktree | 破坏性,全在 Agent 权限之外 |
...
### 更正:OUT-OF-BOUNDS 有两种成因,出口不同(2026-08-20 当日修正)

上表用**未提交脏文件**造的局,于是得出「唯二真出口全是破坏性」。这个结论**对另一半成因是错的**。第三条解法(重钉 base)来自**另一条会话 2026-08-19 在 Dartify 现场实测后写进本机项目记忆的「解法三步」**(`agent-on-guard-deadlock-recipe`,不在本仓内);本案当日复测确认它成立,并测出它**只对一半成因有效**:

| 成因 | `edit --base <自身分支头>` | 出口 |
|---|---|---|
| **已提交发散**(`base...HEAD`,死分支 behind 100+ 横跨全仓) | **PASS** | 有:重钉 base ⇒ `diff(base,HEAD)` 归零 ⇒ 越界清零。**非破坏、权限内、零工作损失** |
| **未提交脏文件 / untracked** | 仍 `OUT-OF-BOUNDS` | 无:`git diff` 与 untracked 不看 base,重钉不影响 |

`parked` 的语义本就是「工作在此搁置」,把 base 钉到自身头与这个语义自洽,不是绕闸。**现场那五条陈年 lane 是死分支的已提交发散,所以它们本来就有非破坏出口**——第一版没分成因,把结论说过头了。

未提交脏文件那一类看似还能「先 commit 变成第一类再重钉」,但**guard 同时拦 `commit` 和 `push`**(`cli/src/guard.rs` 把 `commit`/`push` 一起列为审计点;本案作者会话当日被实拦一次,报错原文「已拦截 git commit/push」),全场没绿之前这条链的第一步就走不了。**真正无出口的是「未提交脏文件 + 多轨共享路径」这一格**,不是整张表。

## 修法

现场按这个顺序试,**从非破坏的往破坏的走**:①worktree 已不存在的幽灵 lane → `worktree forget --id X` 秒清 ②worktree 还在、越界来自**已提交发散** → `edit --base <该 lane 分支头>` 重钉(detached 的钉 worktree 真实 HEAD,钉分支头会让 diff 炸开)③剩下的按债务交单,清理走用户拍板;自己的活换非阻塞路径推进。

**别按文档回填 owns**——回填是把一条 FAIL 换成另一条,而且死分支的 diff 横跨全仓,碰谁撞谁,是打地鼠。
...
**外加一条元教训(本案自身踩的)**:判「无出口」之前先把 FAIL 的**成因分类穷举完**。本案第一版拿未提交脏文件造局,就断言「唯二真出口全是破坏性」;而同一条 FAIL 由已提交发散引起时,`edit --base` 就是干净出口。**同名 FAIL 不等于同一个病**,出口是按成因分的——少分一类,就会把「我没找到」写成「不存在」,然后把一个可解问题上报成需要用户拍板的破坏性决策。

...
> **minor**（用户 2026-09-26 在本会话拍板：`cli/src/**` 属硬停第 1 类）：不动手不坏——`--push` 从任何分支、任何 worktree 都推 origin 的默认分支，推送被拒不留本地 tag，有人在班时只有值守能跑。不算 patch，是因为 `--push` 推到哪里变了，用户该知道。没有 breaking，不需要迁移注记。
...
- **`boot/settlement.md` 机械助手一条改写**：原文警告「`--push` 推的是当前分支同名的远端分支，worktree 分支上别带」，改成现行行为；保留旧二进制的例外和认法（打印的下一步还是 `HEAD <tag>` 的就是旧的）。

### 证据

- 先写测试、看它因为对的原因红：会话 worktree 里 `--push`，remote main 停在 c1（`1ac5138`），tag 却落在 c3（`4567041`）；main 已被别的会话推进时推送照样返回 0（推去了同名分支）；下一步提示是 `HEAD v0.1.1`；没有 origin 时先打了 tag 才撞上 `fatal: 'origin' does not appear to be a git repository`，tag 留在本地；路由闸对 `agent-on tag-release … --push` 返回 `None`。改后全绿。撤 tag 另做变异检验：把 `tag -d` 换成 `tag -l`，「被拒不留本地 tag」那条即红。
- `cargo test` 278 项全过（exit 0，新增 4 条），`cargo clippy --all-targets -- -D warnings` exit 0，`cargo fmt --check` exit 0，`intake-lint` 207 张卡通过，`python3 .github/scripts/check_docs.py` PASS。
- 真二进制端到端（临时仓：裸远端 + clone + `claude/x` 的 linked worktree）：装着的 v0.24.3 二进制 `--push` 之后 remote main 停在 `436467a`、会话在 `d4d1fe4`，远端多出 `claude/x`；新二进制推完 main 等于 HEAD，没有多余分支；main 先被推进时退 1、远端不动、本地 `v0.1.1` 已撤；不带 `--push` 打印 `git push --atomic origin HEAD:main v0.1.1`。
...
> **patch**（用户 2026-09-26 在本会话拍板：`.github/**` 属硬停第 1 类；「不用知道」——只动本仓自己的 CI 闸，产品交付面不动）：旧判据把散文里的「推荐 pin」连同同一行下一个版本号读成第二个推荐 pin，v0.23.3 发版因此红过一次，当时靠改散文绕过去。本版修判据本身，并把那句散文改回原话。

- **CI 文档闸的推荐 pin 判据只认声明写法**：`.github/scripts/check_docs.py` 的 `PIN_RE` 原来认「推荐 pin」之后同一行任意一个反引号版本号，README 路线段一句「CI 推荐 pin 闸在 tag 到达前 checkout 红了一次）。**`v0.23.2`**…」就被读成第二个推荐 pin。v0.23.2 发版时两个号恰好相同才没红，v0.23.3 发版撞上，当时靠改散文绕过去（`fab4796`）。现在版本号必须紧跟标签，只认 ``推荐 pin：`vX.Y.Z` `` 与装机表 ``| **推荐 pin** | **`vX.Y.Z`** |`` 两种写法。判据一收紧，写法走了样的那处声明会静默掉出视野（装机表去掉加粗、留个旧号，闸照报「三处一致」），所以每个文件的声明处数一起钉死：AGENTS 1 处、README 2 处，对不上照违规报。另加四条判据自检样本（三处真声明 + v0.23.3 撞上的原句），每次跑闸先过自检，认错了按「取证失败」退 2。退出码语义（0 / 1 / 2）与「每条都带出口」不变。
- **README 路线段 v0.23.1 注改回原话**：「CI 的 pin 闸」改回「CI 推荐 pin 闸」。那是 v0.23.3 为绕假阳性改的，判据修好就不需要了；原句留在真 README 里，main 上每跑一次 gate 都在复验这条修复。

### 证据

- 先写自检、判据不动：`python3 .github/scripts/check_docs.py` 退 2，报的正是 v0.23.3 撞上的那句（`应认出 []，实际认出 ['v0.23.2']`）；改 `PIN_RE` 后 PASS，退 0。
- 临时仓复现（`git archive` 出来的独立 repo，没开 worktree，碰不到别的会话的闸）：散文改回「CI 推荐 pin 闸」，旧闸原样报 `README.md 自己就有 2 个不同的推荐 pin：['v0.23.2', 'v0.23.3']` 退 1，新闸 PASS 退 0。
- 收紧的代价实测过：装机表那行去掉加粗、写个旧号 `v0.22.0`，旧闸红，只改判据的新闸却照报 PASS——所以加了处数钉死，同一场景退 1。
- 其余几闸没被带松：文件内两个号、跨文件漂移、推荐 pin 没 tag、声明被删、多出一处，都照旧退 1；判据退回旧写法触发自检退 2；`AGENT_ON_DOC_GATE_OFF=1` 退 0；不在 git 仓里退 2。
- rebase 到 v0.24.1 之后、改推荐 pin 之前跑 `python3 .github/scripts/check_docs.py`：`DOC-GATE: PASS（215 份 markdown：职责边界棘轮 / 相对链接 / 推荐 pin 三处一致）`，退 0。
...
> **minor**（用户 2026-09-26 拍板：`cli/src/**` 属硬停第 1 类，带测试与独立复核证据逐项问过后批准合入发版）：不动手不坏——`doctor` 多两段只读输出；同文件闸拦截文案多一行，判据不变，`check --json` 只加可选字段；pre-push 多拦一种本来就违规的动作，有冲突的、判不了冲突的、没有同仓进默认分支 PR 的一律放行；`tag-release --push` 改一条原子推送；已装的 hook 脚本不用重装。无 breaking，不需要迁移注记。**建议动手的一件**：跑一次 `agent-on doctor`，执行面落后就照它给的修法先重编、再刷插件缓存（见下「本机实测」一条）。

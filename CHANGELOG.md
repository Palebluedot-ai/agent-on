...
### feat(kit)：真相之页「开发史」板块——开发日历 + 平台署名 + 逐 commit 时间线（2026-07-18，用户点单）
- **kit/dashboard-template.html** 新增第六 tab「开发史」：日历热力图（每格一天，颜色=当日主力平台，深浅=提交量，点格子锚点跳当天明细）+ 倒序逐日时间线（类型徽章 功能/DEBUG/拍板/文档/收件… + 平台徽章 Claude Code/Codex/Grok/未标注）——历史开发的功能态与 debug 态从此可视，与「里程碑」分工：里程碑人挑大事记，开发史机器转录全量流水
- **真相源 = git log 机械转录**（铁律①最省心的一块）：模板内附一条重绘命令与转录规则——类型读 commit 前缀，平台读 Co-Authored-By 署名（Claude Code 默认带署名；Codex/Grok 在各自提交模板加同款署名即被认出；读不到标「未标注」）
- 配套：顶部数据源清单与页脚补「开发史 ← git log」行；配色守「不发明新色相」纪律全取既有变量；instantiated-from 版本戳由硬编码 v0.4 改为「[pin 版本]」占位符
- 证据：本仓 103 commit 实测转录渲染（Claude Code 74/未标注 29，5→7 月日历、锚点跳转、375px 手机宽、控制台零错）；占位模板自身渲染同验

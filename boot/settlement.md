...
4. **收尾四件**(缺一 = 消化失败;**tag 是硬门,不是可选项**):
   ...
   - **发版硬门(2026-08-02 立;2026-08-03 扩到本仓一切对话交付,防下游「无可升级版本」)**:
     ...
     - 机械助手:`agent-on tag-release --level patch|minor|major --title "一句话" --push`(工作区干净、已 commit 后跑;需 `cargo install --path cli`)。`--push` 把 HEAD 推到 origin 的**默认分支**(认 `origin/HEAD`,失效时退回 main / master),和 tag 一条原子推送,在 worktree 分支上也一样;认不出默认分支就不打 tag。推送被拒(默认分支已前进、同名 tag 已被别人发了)会撤掉刚打的本地 tag,照提示 `git fetch origin`、rebase 后重跑。**旧二进制例外**:打印的下一步若是 `HEAD <tag>` 而不是 `HEAD:<默认分支> <tag>`,它的 `--push` 仍推当前分支同名的远端分支——先重装,或不带 `--push`、打完 tag 手工 `git push --atomic origin HEAD:main <tag>`

...
1. 默认新增 **capability**(给现有 agent 加本事),**不新增永久 role**。
   非要设角色,先过**三格判据**(元原则七):它落在 ①**权限收口点**(谁能碰什么;必须带闸)②**一次性派工词**(这一轮怎么干;用完即弃)③**机械闸**(违规时谁拦)——哪一格?三格都落不进 = 装饰,别设。
   角色名只是这三格的可读别名:`maintainer`=①,`值守`=①+③,`独立审查员`=②。**说不出它守哪条判据的角色,就是装饰**(实证:Gen-1 的 Curator/Worker 死得最干净,因为背后根本没判据)。

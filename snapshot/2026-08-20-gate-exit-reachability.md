...
第三条解法（重钉 base）来自**另一条会话 2026-08-19 在 Dartify 现场实测后写进本机项目记忆的「解法三步」**（`agent-on-guard-deadlock-recipe`，不在本仓内）；本会话是在核对一条与它冲突的记忆时读到的，当日复测确认成立：**OUT-OF-BOUNDS 有两种成因，出口不同。**

（本轮增量只有两样：复测确认那招成立，以及测出它**只对已提交发散有效、对未提交脏文件无效**这个成因二分。解法本身不是本会话原创，也不是值守给的——初版把来源记成「值守回执带来」是错的，2026-08-20 当日更正。）

| 成因 | `edit --base <自身分支头>` | 出口 |
|---|---|---|
| 已提交发散（`base...HEAD`） | **PASS** | 有：重钉后 diff 归零、越界清零，**非破坏、权限内、零工作损失** |
| 未提交脏文件 / untracked | 仍 `OUT-OF-BOUNDS` | 无：`git diff` 与 untracked 不看 base |

上表是拿**未提交脏文件**造的局，所以得出「唯二真出口全是破坏性」。**现场那五条陈年 lane 是死分支的已提交发散，它们本来就有非破坏出口**——第一版没分成因，把结论说过头了，已在案 40 与 gc-pattern 更正。

元教训：**同名 FAIL 不等于同一个病。** 判「无出口」之前要把成因分类穷举完，否则会把「我没找到」写成「不存在」，进而把一个可解问题上报成需要用户拍板的破坏性决策。

另修正一条流传中的错误口径：「guard 只拦 push 不拦 commit，所以抢救永远可先做」——**不成立**。`cli/src/guard.rs` 把 `commit` 与 `push` 一起列为审计点，本会话当日就被实拦一次（报错原文「已拦截 git commit/push」）。全场没绿之前，「先 commit 把脏文件变成已提交发散、再重钉 base」这条链的第一步就走不了。

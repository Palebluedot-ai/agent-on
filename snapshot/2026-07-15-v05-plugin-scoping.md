...
v0.5 形态**可行且有活参照**（superpowers 就是单源多工具 plugin），但研究挖出**三个会卡实施的真实风险**（其中一个可能让 Codex 侧 hook 打包直接不成立）——故 v0.5 不是「照抄就发」，是「低风险件先建 + 高风险件设测试闸门」。**用户 2026-07-15 拍板路线 A（分期发）。2026-07-16 封 `v0.5.0`**：阶段 1–2 + 4 + 可移植路径 + 上游贡献 + README 五分钟装机；Claude install/hook 注册实测通过；Codex #16430 与交互真拦诚实写在 CHANGELOG 边界，不挡 minor 封版。**
...
## 阶段 2–4 进度（2026-07-15 同日续）

| 阶段 | 状态 | 证据 |
|---|---|---|
| 2 Claude guard 入 hooks | **仓内 ✅** | `hooks/hooks.json` + guard `AGENT_ON_ROOT`；`claude plugin validate` 通过；个人 scope 路 A 不删 |
| 2 闸门：Claude install + hook 注册 | **✅ 2026-07-16** | 见下「Claude 闸门实测」 |
| 2 闸门：交互会话内 PreToolUse 真拦 | **部分** | CLI `-p` 本机报 `Not logged in`，未跑到 Bash 工具；guard 自 cache 路径合成 PreToolUse JSON 已真拦（exit 2）；plugin hook **已注册**有 debug 证据 |
| 3 Codex guard 接线 | **备件 ✅ / 接线 ❌** | `hooks/hooks-codex.json` 已写；manifest 仍 `hooks:{}` |
| 3 闸门：#16430 hook 真执行 | **未过** | 过了才把 codex plugin.json 的 hooks 指到 hooks-codex.json |
| 4 换机/文档 | **✅** | README A/B 路、codex/README、skill `$ROOT` 解析、hooks/README、kit/guard 两路注册 |

**产品诚实点（阶段 4 落笔）**：plugin cache 是只读模板面；**结账 intake 必须写可写 clone B**。

**可移植路径（2026-07-16 拍板落地）**：废除产品默认 `~/Projects/Agent-On`。A=`CLAUDE_PLUGIN_ROOT`；B=`AGENT_ON_ROOT` → `~/.config/agent-on/config.json` → lock「本地路径」；未登记 B 时 guard fail-open、settle/digest 拒绝。实现 `kit/guard/agent_on_paths.py` + skill doctor。

## Claude 闸门实测（2026-07-16，本机）

**步骤**：临时 `mv ~/.claude/skills/agent-on` 为 parked → `claude plugin marketplace add /Users/chao/Projects/Agent-On` → `claude plugin install agent-on@agent-on -s user` → 验证 → 恢复 symlink + 恢复个人 scope guard。

| 检查项 | 结果 | 证据 |
|---|---|---|
| marketplace add | ✅ | `Successfully added marketplace: agent-on`；`known_marketplaces.json` source=directory path 本仓 |
| install + enable | ✅ | `agent-on@agent-on` Version 0.5.0 enabled；cache `@ gitCommitSha 5cbc630` |
| component inventory | ✅ | `claude plugin details`：Skills (1) agent-on · **Hooks (1) PreToolUse** |
| skill 别名 `skills/agent-on → ../skill` | ✅ | cache 内 readlink 到 `…/0.5.0/skill`，SKILL.md 可达 |
| hooks.json 被加载 | ✅ | debug：`Read hooks.json for plugin agent-on (enabled=true): /Users/chao/Projects/Agent-On/hooks/hooks.json`；`Loading hooks from plugin: agent-on`；`Registered 6 hooks from 4 plugins` |
| 目录 marketplace 读活路径 | 注 | directory source 时 hook/skill 直接读本仓路径（非仅 cache）；cache 仍落盘完整副本 |
| guard 自 `${CLAUDE_PLUGIN_ROOT}` 路径 | ✅ 合成 | `CLAUDE_PLUGIN_ROOT=$cache/0.5.0` 跑 hooks.json 同款命令：跨仓 commit → exit 2 + 中文回灌；allow-self / allow-log → 0 |
| 交互会话 PreToolUse 真拦 | ⚠ 未完成 | `claude -p` → `Not logged in · Please run /login`；需登录后的交互会话补一次（个人 scope 或 plugin 任一拦到即可） |
| symlink 双挂 | 并存 | 测后**恢复** `~/.claude/skills/agent-on`；debug 曾出现 plugin skill skip（user-level 已 surface）——hooks 不受影响 |
| 个人 scope 路 A | 恢复 | 测中曾 park PreToolUse 条目以隔离；结束后已写回；plugin 与路 A **双挂** |

**本机当前 Claude 态（测后）**：plugin `agent-on@agent-on` enabled + marketplace directory 本仓 + skill symlink 恢复 + 个人 scope guard 恢复。

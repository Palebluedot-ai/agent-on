...
v0.5 形态**可行且有活参照**（superpowers 就是单源多工具 plugin），但研究挖出**三个会卡实施的真实风险**（其中一个可能让 Codex 侧 hook 打包直接不成立）——故 v0.5 不是「照抄就发」，是「低风险件先建 + 高风险件设测试闸门」。**用户 2026-07-15 拍板路线 A（分期发）。阶段 1–2 仓内 + 阶段 4 文档已齐；Claude 端到端 install 闸门已于 2026-07-16 本机实测通过（见下「Claude 闸门实测」）；阶段 3 Codex #16430 仍开。**

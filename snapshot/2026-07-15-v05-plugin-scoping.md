...
v0.5 形态**可行且有活参照**（superpowers 就是单源多工具 plugin），但研究挖出**三个会卡实施的真实风险**（其中一个可能让 Codex 侧 hook 打包直接不成立）——故 v0.5 不是「照抄就发」，是「低风险件先建 + 高风险件设测试闸门」。**用户 2026-07-15 拍板路线 A（分期发）。阶段 1 + 阶段 2 仓内落地 + 阶段 4 文档已齐；阶段 3 Codex guard 接线与 Claude 净机 install-test 闸门仍开。**
...
| 2 闸门：CLAUDE_PLUGIN_ROOT 注入 + 真拦 | **未过** | 需净机/摘 symlink 端到端；本机不并行测（避免双挂） |

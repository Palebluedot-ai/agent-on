...
## §4.5 社区 babysit 生态：存在，但和本仓值守是不同物种

社区确实有一套 `babysit-pr` skill 生态（`novotnyllc/babysit-pr` 打包成 Claude/Codex 双插件，源出 **openai/codex** 的 `.codex/skills/babysit-pr`，Apache-2.0；另有多个 gist / marketplace 转载版）。装法：
...
读过它的 SKILL.md 原文后，结论是**不冲突、也不能替代**——两者站在 PR 的两侧：

| 维度 | 社区 `babysit-pr` | 本仓值守 |
|---|---|---|
| 站在谁那边 | **作者侧** | **维护者侧** |
| 管几条 | **一条 PR**（从当前分支推断） | **整个 open PR 队列** |
| 代不代修 | **代修**：诊断 CI 失败 → 本地打补丁 → commit → push；还会回复 review 评论并 resolve 线程 | **零代修**：真缺陷带证据打回作者会话 |
| 合不合 | **不合**，终态是 merged/closed 由别人达成 | **合**，且合并权在班期间唯一归它 |
| 授权模型 | 无分级；flaky 重试 3 次预算 | 默认合入档 / 必须先问档两张显式清单，fail-closed |
| 撤销面 / 记账 | 无 | 有（可删/不能删两栏 + 台账） |

**可复用的两点**：①「绿了也不停表，继续盯后到的 review 反馈」这条不变量值得抄进值守——本仓现在合完就结单；②CI 失败分类（branch-caused vs flaky）的判据可以进 §4 分诊手册（本仓无 CI，先记着给 Dartify 用）。

**一条必须写明的碰撞**：如果作者会话在自己 PR 上跑 `babysit-pr`，它会**往分支 push 修复**——head SHA 一动，landing 快照的证据键 `(head_sha, base_sha)` 即失效、值守要重新取证与追平。两边同时开着不会出错，但会让值守白跑几轮。**建议**：要用就在交单**之前**用（PR 开出来交单之后，分支归值守的核对面管）。

至于 AgentOS 类框架（`buildermethods/agent-os` v3 等）：强项是 spec-driven 与 standards 注入，与 BOOTSTRAP / phase 卡定位重叠；弱项（治理、授权分级、合并权归属、撤销面）恰好是本仓强项。按 AGENTS.md「不引入与 GStack / Superpowers 重叠的环节型功能」，**不引入**。
...
3. **§1 子节内再补「末尾 Summary 块」**：长轮次可选的五行压缩块（已完成 / 待拍板 / 交接值守 / 下一步建议 / 球在谁那），格式固定。两条硬要求：不许引入正文没有的新信息；「球在谁那」并进 Summary 末行**不另起一行重复**（两个尾巴会让用户不知道看哪个）。§6 同步加了一句指回来。
4. **§3 新增子节「默认值默认等于建议值」**：源自用户 2026-08-19 原话「为什么我不回按照不是建议的来，看起来很奇怪」。规则是默认值默认写建议值，只有**不可逆**或**超出已授权范围**两种情况允许降级且必须写明理由。理由：建议与默认系统性不一致会让用户学到「沉默 = 较差结果」，被迫每条都回，抵消拍板前移的全部收益。§11 自查表同步加一条。
...
可验证断言 31 条，逐条跑命令核对：**28 条确认 / 3 条改正**。三条改正都是硬伤级（不改会误导决策）：

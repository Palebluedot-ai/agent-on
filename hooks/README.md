...
| matcher | 判什么 | 拦什么 |
|---|---|---|
| `Bash` | ①跨仓 git 边界 ②同一未提交文件（只在 `commit`/`push`，**只判本树**：另一棵树 7 天内也改过同一路径才拦；不读 lane 登记，2026-09-24 起）③**跨窗口指令路由**（值守在班时的合并 / 对外通信命令；值守窗口的每次调用顺手续在班心跳） | 越界 git 写；本树的 `blocked:`；非值守窗口的 `gh pr merge`、`update-branch`、tag push、PR/Issue 评论、chat webhook |
| `SendMessage` | **跨窗口沟通归属**：收件人是不是**另一个已登记窗口**（地址前缀匹配某条 lane 的 worktree 目录名） | 非值守窗口发给另一个窗口的横向消息；交单 / 回执发给值守、以及 `main` / 子代理等会话内部地址照旧放行 |

路由闸只在**有人 `agent-on oncall claim` 且登记还活着**（默认 90 分钟内有心跳）时生效，无人在班或登记过期一律 fail-open；协议全文见 [`kit/babysit/ROUTING.md`](../kit/babysit/ROUTING.md)。
...
- **低开销**：非 git 与 git 读命令在 Rust 解析后立即返回；只有 `commit/push` 才比较各工作树的未提交文件。

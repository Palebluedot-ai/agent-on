...
# 职责边界:只拦一件事——「会话根不在 agent-on 仓内,却对 agent-on 仓执行 git 写操作」。
...
# 注册:Claude Code ~/.claude/settings.json 与 Codex ~/.codex/hooks.json 的 PreToolUse(同一 schema)。
# 语义:exit 0 = 放行;exit 2 = 拦截,stderr 回灌给模型说明原因(两家等价)。
# 回滚:从两处 hooks 注册里删掉本脚本条目即回 advisory 现状;脚本本体无副作用可留。
# 来源:deep-research run wf_9c47f385-3e2(2026-07-13)+ Euan 越界事故(5b4ecdd);卡 enforcement-layer-design。
import json
import os
import re
import shlex
...
# 命令文本里的 agent-on 路径形态(~ 或 / 开头才算路径,防误伤 commit message 里提到 agent-on)
...
def main() -> int:
    ...
    # 命令文本里显式出现的 agent-on 路径(cd/-C 之外的形态,如重定向、xargs)也算目标
    ...
    if inside_agent_on(os.path.realpath(os.path.expanduser(sess))):
        return 0  # agent-on 仓自己的会话(含 .claude/worktrees/ 下的),放行

    sys.stderr.write(
        "⛔ 跨仓 git 边界拦截(agent-on-git-guard):项目端会话对 agent-on 仓只写 intake/ 素材文件,"
        "不 add / 不 commit / 不 push(AGENT.md 硬规矩 2026-07-13)。\n"
        f"会话根 {sess} 不在 agent-on 仓内,却试图对 agent-on 仓执行 git 写操作。\n"
        "正确动作:落盘 intake 文件即止;git 收件/消化由用户切到 agent-on 仓会话执行。\n"
        f"被拦命令:{cmd}\n"
    )
    return 2

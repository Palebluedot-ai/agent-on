...
> 授权分级与时延目标的唯一真相在 [MERGE-POLICY.md](MERGE-POLICY.md)：本页第 5 条的两张清单照那页 §3/§4 抄定具体值，不在两处各自展开。
...
5. **三条边界**：
   ...
   ② 授权分级**两张清单都要显式写死**（只写「可自主」不算数）——
      **默认合入档**（用户预授权，全绿即合、不问、合完记一行）：<五类，照 agent-on
      kit/babysit/MERGE-POLICY.md §3 抄定，按实际 diff 判不按标题判>；
      **必须先问用户**（清单外一切也归这栏）：<清单，照同页 §4 抄定>；
      默认合入档从交单到合入的中位时延目标 ≤ <X> 分钟，超时进下班交接（算法见同页 §5）；
      用户没明确授权过则默认合入档不生效，全部按「必须先问」办；

...
agent-on oncall heartbeat                                          # 显式续心跳（值守窗口的每次工具调用会自动续，这条是保险）
agent-on oncall release [--force]                                  # 下班
```

**登记只在窗口活着时算数（2026-09-14 起）**：`claim` 写下 `heartbeat_at`；值守窗口每一次经 PreToolUse guard 的工具调用（任何 Bash / SendMessage）自动续心跳（一分钟内最多写一次）。心跳超过 `oncall_stale_after_minutes`（默认 **90** 分钟；`<common git dir>/agent-on/config.json` 可配，`0` 关掉）就**失效**：闸 fail-open，`claim` 不用 `--force` 即可接班，`status` 写明「N 分钟没有心跳」。值守窗口只是安静了很久、人还在的话，下一条命令会把自己续活；别的窗口已接班（记录被覆盖）则不会误续。
...
**登记失效自动降级**：登记的 worktree 已不存在，**或超过失效窗口没有心跳**（窗口关了没下班——这才是常见的那种）→ 按「无人在班」处理，避免死锁全场；`oncall status` 会显示「登记已失效」与原因；被拦的功能窗口文案里也带「值守最近心跳 N 分钟前；M 分钟没心跳自动失效」。
...
- **横向消息闸只认台账里的窗口**：一个**未登记 worktree** 的窗口不在 lane 台账里，发给它的消息拦不住。2026-09-14 起边界闸也不再连坐未登记树，所以这条通道只有纪律层兜着；要机械兜住就给那棵树 `claim` 一条 lane。同理，会话名与 worktree 目录名完全无关的窗口也认不出来（本机的命名惯例是同源的，别的机器未必）。
- **只在命令位置认命令名（2026-09-26 起）**：第一个词、`&& || ; | &` 之后、`env` / `sudo` / `timeout N` / `xargs` 这类包装之后的那个词才算命令；参数位置的 `mail`、`slack`、`git push` 是数据。此前逐词比对，`grep -w mail`、`ls /usr/share/teams` 都被判成对外通信（Dartify 2026-09-26）。命令位置之外的误伤是**闸的 bug，按 bug 报**（附命令原文与 exit 码），不要教功能窗口改写命令绕开——那是把闸训练成噪音。`bash -c "…"`、`$(…)`、子 shell 里的命令闸看不见，属于 deny-list 的已知盲区。
...
- **fail-open 是故意的**：无人在班、或登记过期时闸完全沉默。代价是「忘了上岗」等于没有闸、值守窗口静默超过失效窗口后会短暂没闸；收益是这套机制永远不会把单人开发或值守下班后的仓库锁死——包括值守窗口关了没 `release` 的那种。

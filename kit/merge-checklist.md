...
- [ ] **7b. 状态面与代码同批收口**:核对 exact HEAD + 测试输出 + progress/phase/dashboard(及计划 TODO 当前态)一致;禁止代码已前进而状态面仍写已关闭 P0 或未来时态。易变计数(ahead/behind)不写入长期真相——push 前 fresh fetch。**合并多个 PR 后**扫一遍 TODO 触发是否已命中未回表(禁靠「顺手」)。

...
- [ ] [回执/契约字段·若本卡改回执、状态码或契约字段]:验收测试调用**下游真实消费者**(recovery / replay / 校验器)的那段代码;只断言生产者写出去的值不算证据(anti-hallucination C 附6 第 2 条)
- [ ] [行为类条目·若验收的是「策略被触发/告警被激发/降级被走到」]:先自问「本测试的参数下该行为**结构上可能发生吗**」——不可能则该条只能记作**链路证据**,行为证据另立条目并写明触发方式(调参/注入/等待),禁止用链路通过冒充行为验证(C 附6 第 8 条)
- [ ] [活体·若接外部系统]:共享配额(同 IP/同 key)的活体验证**串行**,证据里注明「当时有无并行外部负载」;接完必须跑一次**覆盖全体对象的完整率审计**,抽样看几根不算验收(C 附6 第 5/9 条)
- [ ] [真钱/凭据·若本卡需真实签名或真钱命令]:本人执行、AI 转录——原始输出回来前标 ⏸,禁止 dry-run 或推断冒充(C 附6 第 10 条)

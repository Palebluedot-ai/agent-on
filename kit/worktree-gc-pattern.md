...
3. **禁止为了让闸变绿而改账**。把 OUT-OF-BOUNDS 清单回填进 owns、直改 lane JSON 扩边界，都是**用假账换绿灯**：闸绿了，互斥保护也没了，而且多棵脏树同时回填必然撞出 OVERLAP——一条 FAIL 换成另一条（2026-08-20 实测，两个 FAIL 条件互为对方的唯一解，可行域为空）。真解是清债或修机制，不是改账。

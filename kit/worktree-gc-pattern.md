...
**2026-09-14 起连坐已作废**（`kit/worktree-control-plane.md`「闸只拦真冲突」）：未登记树、幽灵登记、无人持有的越界都不再拦任何人的 commit，下面这张阶梯从「解锁配方」降为「清仓库卫生的顺序」——债务口径与清账责任照旧。①②③ 来自另一条会话 2026-08-19 在 Dartify 现场实测的「解法三步」，2026-08-20 由本仓复测确认，并补上「② 只对已提交发散有效」这条适用边界：

| 顺序 | 情形 | 动作 | 破坏性 |
|---|---|---|---|
| ① | worktree 早已不存在，只剩 lane 记录（幽灵；`check` 报 `MISSING`） | `agent-on worktree forget --id X`（任何状态都可） | 无 |
...
**「先 commit 再说」现在成立了**：guard 只拦本树未提交改动进别人活轨 owns 的那一种情形，别的树红着不影响你抢救自己的工作（旧版 ≤ v0.19.0 两个都拦，那是连坐时代的口径）。

...
> **去向：landed@v0.14.0** —— kit/babysit/ 四件落位 + landing/worktree 控制面补节 + BOOTSTRAP/adopt 接线；建议归宿 §三 1–3 全落位，§三 4（CLI `worktree edit` / `claim --force-redivide`）已立后台任务卡（v0.15.0 消化批，与 --owns 逗号 bug 同卡）——本文件全部去向收口。

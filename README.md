...
**Q：这台电脑坏了，怎么恢复？**
换机三步：① `git clone` 本仓到任意路径（GitHub 远端就是异地副本）；② `ln -s <仓路径>/skill ~/.claude/skills/agent-on` 挂回斜杠命令；③ 全局 CLAUDE.md 的口令路由随 claude-memory 仓同步自带，不用手配。用 Codex 的机器再加一步：④ 按 [codex/README.md](codex/README.md) 挂两件（AGENTS 片段 + prompts symlink）。各项目的 lock / loop-notes 在项目自己的仓里，跟项目走，与本仓互不牵连。

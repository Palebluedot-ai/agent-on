...
| `bench/` | **Bench** | 翻车案例集 24 卡（[bench/cases/](bench/cases/README.md)，Euan 实战 21 + IPONews 1 + 前身仓标本 2）+ 能力探针 + 能力真相表 + 修正闭环 |
...
不会自动建——它是个 git 仓，`git clone` 到**任意路径**都行。路径锚点：全局路由（agent-memory 的共用真相 AGENT.md）、各项目 lock 的「本地路径」、可写工作仓（默认 `~/Projects/Agent-On`，可用 `AGENT_ON_ROOT` 覆盖）。装机两条路见下「换机」。

**Q：这台电脑坏了，怎么恢复？**
换机任选一路（**plugin 与 symlink 可并存**，存量项目不必立刻迁）：

| 路 | 步骤 |
|---|---|
| **A · Plugin（v0.5 新默认）** | ① `git clone` 本仓到可写路径（结账 intake 需要）② Claude：`/plugin marketplace add <仓路径>` → `/plugin install agent-on@agent-on`；Codex：`codex plugin marketplace add <仓路径>` → `codex plugin install agent-on@agent-on` ③ 全局口令路由仍随 agent-memory `setup.sh`；路径非默认时 export `AGENT_ON_ROOT=<仓路径>` |
| **B · symlink（零改动兼容）** | ① clone ② `ln -s <仓路径>/skill ~/.claude/skills/agent-on` + `ln -s <仓路径>/skill ~/.agents/skills/agent-on` ③ agent-memory `setup.sh`；guard 个人 scope 见 [kit/guard/README.md](kit/guard/README.md) |

诚实边界：Claude 侧 guard 可随 plugin 自动挂（`hooks/hooks.json`）；**Codex 侧 guard 暂仍靠个人 scope**（plugin hook 执行闸门 #16430 未过，manifest 保持 `hooks:{}`）。各项目的 lock / loop-notes 在项目自己的仓里，跟项目走。
...
- **v0.5（进行中 2026-07-15）**：Plugin 打包——阶段 1 骨架 ✅ · 阶段 2 Claude guard 入 `hooks/` ✅（净机 install-test 闸门待过）· 阶段 3 Codex guard 备件已写、接线等 #16430 · 阶段 4 换机/文档 ✅ 主路径。调研见 [snapshot/2026-07-15-v05-plugin-scoping.md](snapshot/2026-07-15-v05-plugin-scoping.md)

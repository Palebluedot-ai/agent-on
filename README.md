...
| `intake/` | — | 承接层：各项目「结账」回流的 Promotion Card 落点（目录即仪表盘） |
| [CHANGELOG.md](CHANGELOG.md) | — | 发布账本；版本真相 = git tag（major=不动手会坏 / minor=不动手不坏 / patch=不用知道） |
...
1. **一代（4-5 月）· Harness Engineering**：想自建编排运行时（Router / Lane / Policy 注入）。5200+ 行设计物（文档+schema+脚本），引擎目录 0 行代码——教训：工具能力已覆盖时，造引擎不如立纪律。活下来的是契约 schema、DoD 门禁、「默认隔离 + 单一写者」原则。
...
- **v0.2（当前）**：批一~批四完成——五块骨架、三代资产迁入、前身仓精选移植、Bench 案例集 17 卡、**迭代闭环六站机制**（playbook/iteration-loop.md：种→采→结→消化→发布→升级，三口令「结账 / 升级 / 消化」）、前身三仓归档
- **v0.3 门槛**：Euan 仓内 agent-on/ 倒仓首结账跑通（闭环验收测试）+ 一个新项目 BOOTSTRAP dogfood 全流程
- **v1.0**：两个以上项目用它跑完完整闭环（Project A 的坑确实变成了 Project B 的免疫）

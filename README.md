...
**Q：这台电脑坏了，怎么恢复？**
换机三步：① `git clone` 本仓到任意路径（GitHub 远端就是异地副本）；② `ln -s <仓路径>/skill ~/.claude/skills/agent-on` 挂回斜杠命令；③ 全局 CLAUDE.md 的口令路由随 claude-memory 仓同步自带，不用手配。各项目的 lock / loop-notes 在项目自己的仓里，跟项目走，与本仓互不牵连。
...
- **v0.2（当前）**：五块骨架、三代资产合流、Bench 案例集 17 卡、迭代闭环六站机制、S/M/L 档位路由、存量项目接入书。**冻结令生效中：首次真实结账跑通前，不新增 playbook 篇目 / kit 模板**
- **v0.3 门槛（唯一在办项）**：Euan 仓内 agent-on/ 倒仓首次结账 + 首次消化跑通（intake 已候着 16 张评审卡）

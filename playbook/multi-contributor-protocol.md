...
**连坐闸另加一条:成本要落在有权限清理的角色身上。** 全场共担的闸(一处坏 → 全场 FAIL)保证账实一致,这个理由成立;但它会把清理成本随机砸给**下一个恰好要提交的会话**,而那个会话通常最没上下文(坏的不是它造的)、最没权限(清理是硬门),还正被闸拦着做不了自己的活。连坐可以留,**但必须配一条常设议程**——由值守/控制轨定期清账(worktree 见 [kit/worktree-gc-pattern.md](../kit/worktree-gc-pattern.md)「陈年树是债务」)。没有常设清账的连坐,等于给全场装了一颗随时间累积、随机引爆的雷。

...
### 三½.5 闸的六张面(触发 / 读取 / 并发 / 出口 / 判据 / 执行)

> 源流:Dartify 2026-08-15——PR-only 闸红着合入、后续 PR 代付;闸读 `git log` 不读工作区导致提交前假绿;直推 main 把自己的 PR 撞成 DIRTY 且 CI 零 job。digest gate-three-faces。出口面增补 2026-08-17:digest gate-error-message-is-a-work-order。出口面第二个必要条件(可达性)增补 2026-08-20:worktree 边界闸连坐死锁实测,见 [snapshot/2026-08-20-gate-exit-reachability.md](../snapshot/2026-08-20-gate-exit-reachability.md) 与 bench 案 40。判据面与执行面增补 2026-09-26:CryptoQuant / aster-agent / Dartify 三项目同族(跨仓闸按文本判越界、值守路由闸按词元判外发、插件缓存里的旧闸还在执行),bench 案 46 / 47。
...
4. **出口面:出口要说得清,更要走得通**。这一面有两个必要条件,缺一条闸就是坏的:
   - **4A 自解释(报错文案即工单)**。拦得住只是及格,拦下之后要能自解释——把修复选项连同具体语法写进报错文案,失败信息应当是可执行工单而不是谜语;被拦的会话(人或 AI)照单开工,省掉一轮跨会话/跨时区澄清(Dartify #169:check_privacy_retention 报错给出两条带语法的修复路线,作者照单选路线②,15 分钟闭环零来回)。
   - **4B 可达性:每个 FAIL 条件至少有一条出口,落在被拦者当下的权限内,且非破坏性**。工单写得再清楚,照做的终点仍是 FAIL,出口面照样是坏的。**出口全是破坏性动作、或全部需要比被拦者更高的权限,这不是闸,是死锁**——它把「拦住」变成「拦死」,而且拦的往往不是肇事者(2026-08-20 实测:worktree 边界闸的两个 FAIL 条件 OUT-OF-BOUNDS 与 OVERLAP 互为对方的唯一解,可行域为空;真出口只剩清空脏文件与删 worktree,全是破坏性、全在 Agent 权限之外,而肇事的陈年树是几个月前别人留下的)。
5. **判据面:判动作的真实目标和命令位置,不判文本里出现了什么**。闸要回答的是「这条命令会对被保护的东西产生什么变更」。跨仓守卫判 git 写操作真正作用的仓(跟踪 `cd`,认 `-C` / `--git-dir` / `--work-tree`;commit message、heredoc 正文是数据);路由闸只在**命令位置**上认命令名(第一个词、`&& || ; |` 之后、`env` / `sudo` / `timeout` / `xargs` 这类包装之后),参数不是命令。把「提到」当「操作」的闸,误伤是结构性的:grep 一个 `mail` 被判对外通信(Dartify 2026-09-26),commit message 引了上游路径被判越界(CryptoQuant / aster-agent,案 46)。误伤成本不对称——被拦的会话按文案转投或拆命令,一条只读查询变成跨窗口往返,还会训练使用者把闸当噪音。
6. **执行面:规则改了 ≠ 在跑的那份换了**。hook 挂的是**某个路径上的某份副本**——插件缓存、settings.json 里的 shim、PATH 上的二进制,各按各的版本判。仓里修好的判据送不到缓存里那份旧逻辑,而且新旧两代可以**并排挂着**,每条命令各判一遍,谁拦算谁(2026-09-26 实测:插件停在 0.5.0、仓里已是 0.12.1,CryptoQuant / aster 撞到的误拦全出自缓存里的老 Python 闸,仓里的 Rust 闸早就按目标仓判了,案 47)。**做法**:发版、升级 pin、自检时对一遍「宿主上实际挂着的 hook 命令」——路径 + 内容哈希,和 READ_ROOT 里的同名文件比,标出对应版本;被拦时先看拦截文案点名的执行体路径。分发上,hook 只转发到 PATH 上的 `agent-on guard`,不在缓存里留一份会过期的逻辑。

**写闸/审闸自检四问**(第三问专治互锁,第四问专治判据面误伤):

1. **谁会被拦?他手上有什么权限?** AI 会话默认没有破坏性权限——删目录、删分支、`--force`、清别人的脏文件、改自己的权限配置,通常全被硬墙拦。出口只要落在这些动作上,对它就等于没有出口。
2. **照报错做完,能不能到绿?** 把出口指令**实际跑一遍**,不是想一遍。「解法看着合理」和「解法能到绿」之间,这次差了一整个 OVERLAP。
3. **两个 FAIL 条件会不会互为对方的解?** 若修 A 必然触发 B、修 B 必然触发 A,可行域为空。这类**互锁 FAIL 对**必须在设计期消除,不能留给现场会话临场解——现场只会在两条 FAIL 之间来回弹。
4. **被拦的这条命令,到底会对被保护的东西产生什么变更?** 答不出具体变更(新 commit / 改文件 / 推 ref / 外发一条消息),就是判据面的误伤——该修的是闸,不是教被拦的人改写命令。

**连坐闸另加一条:成本要落在有权限清理的角色身上。**(2026-09-14 补记:worktree 边界闸已**不再连坐**——红灯只算到当事那棵树头上,别的树的状态只进报告;判据改成「此刻有没有人在这里写」而不是「有没有一条记录」,见 bench 案 45 与 [snapshot/2026-09-14-gate-one-rule-and-oncall-heartbeat.md](../snapshot/2026-09-14-gate-one-rule-and-oncall-heartbeat.md)。下面这段留给仍要用连坐的闸。)全场共担的闸(一处坏 → 全场 FAIL)保证账实一致,这个理由成立;但它会把清理成本随机砸给**下一个恰好要提交的会话**,而那个会话通常最没上下文(坏的不是它造的)、最没权限(清理是硬门),还正被闸拦着做不了自己的活。连坐可以留,**但必须配一条常设议程**——由值守/控制轨定期清账(worktree 见 [kit/worktree-gc-pattern.md](../kit/worktree-gc-pattern.md)「陈年树是债务」)。没有常设清账的连坐,等于给全场装了一颗随时间累积、随机引爆的雷。
...
分支保护开 require-up-to-date 硬门后,「合并」变成全局串行资源:N 条会话各自追平自合 = 每次合并把其余人打回 BEHIND,全场 O(N²) 次 rebase;单一值守窗口串行调度 = O(N)。**合并权中央化不是偏好,是硬门下的最优解**——功能会话开完 PR、首轮 CI 触发、描述写全、交单 = 交付完成,不自己合;追平一律走托管平台服务端 update-branch API,不碰任何本地 worktree。接入件与值守文档模板见 kit/babysit/。五条配套纪律:

1. **批准的来源只有一个**:外向硬门动作(合并/删除/迁移)的用户批准必须来自**本会话内的用户输入**。同行会话转述的用户原话再可信也只是情报——注明转述来源、向本人确认后才执行。结果大概率一致,但这一步是权限模型的地基(#169 实测,作者会话回执「该省的从来不是这步」)。2026-09-26 补:批准的冒牌不止转述一种,还有**过期的**(上一批活里的授权拿来覆盖这一批)和**外延的**(授权的是 A,被读成也授权了 B),三种都不算数(kit/babysit/MERGE-POLICY「批准的两种载体」,bench 案 50)。
...
4. **转述 ≠ 出处:解法的来源也要记准**(2026-08-20 增补)。多窗口协作里,一条解法常常是**经中转到手**的——从别的窗口的回执、从另一条会话写下的项目记忆、从用户转述里读到。落进 canonical 时**必须指到真来源**,不能指到中转站。记错的代价不是面子:本仓文档是证据链,后面顺着出处回溯的人会**走空**,而走空的人无从判断这条解法当初在什么条件下被验证过。写法:出处写「谁、什么时候、在哪个现场实测的」,再单列**本轮的增量是什么**(复测?补了适用边界?还是原创)。与第 1 条同源——**批准的来源只有一个,出处的来源也只有一个**;区别是批准转述了要复核,出处转述了要溯源。
5. **开 PR = 进合并队列;等 CI 要对准 head SHA**(2026-09-26 增补,Dartify 两卡 opened-pr-is-queued-for-merge / ci-wait-must-key-on-head-sha)。调度只看形式面(CI 绿 + 模板写全),不知道作者还在复核:#199 开出后作者起了一轮五视角复核,值守按首轮 CI 绿先合了,复核查出 8 条 confirmed(含一条闸自身盲区),只能走 follow-up #204。**合并前复核要么开 PR 之前跑完,要么开 draft,复核过了再转 ready**。等 CI 同理:`update-branch` 返回时新 merge commit 已是 head、仓内 CI 还没起跑,`gh pr checks` 上只挂着一个不依赖 checkout 的检查,按「没有 pending 就算完」写的等待循环 30 秒内报了全绿(#263)。**先记 head SHA,按 `headSha` 确认这个 head 的 run 已 completed,再看 checks 与 `mergeStateStatus`**。这条 08-16 已落在值守模板和案 37,功能会话读不到,在自己写的等待循环里又犯了——规矩要落在**动作方**读的那份文件里(§三½.8 末段同一个教训),所以这次也写进了 kit/babysit/CONTRIBUTING-CLAUSE 第 2 条。

### 三½.7 外向硬门的边界(push 与开 PR 不在内)

> 源流:2026-08-19 用户实测。一条功能会话把活干完、commit 完,停在「要我推上去开 PR 吗?推 GitHub 是外向操作,等你点头」;用户反问「我之前一直都是自己推了,为啥还需要我点头」。查证:那句话**既不出自本方法论,也不出自项目宪法**——是宿主(Claude Code)每个会话自带的通用安全句「对不可逆或外向操作先确认」被当成了项目制度。而该项目宪法的 worktree 与分支生命周期一节恰恰**要求**本地独有工作 48h 内上远端(开 draft PR 也行),理由是「机器一坏就丢」。**收紧到了反方向。**

**判据一条就够:这个动作改的是本轨内部状态,还是全场共享状态 / 外部世界?**(与 kit/babysit/ROUTING.md 路由表同一条判据,那里管「值守在班时谁执行」,这里管「要不要先问用户」)

| | 内容 | 谁决定 |
|---|---|---|
| **本轨内部动作** | 提交 · 推**自己的**分支 · 开 PR / draft PR · 跑测试 | 自己做,**不问** |
| **外向硬门** | merge · tag · release · 直推受保护分支 · force-push · 删远端分支 · 关别人的 PR · 对外发言(PR·Issue 评论 / 邮件 / IM / webhook)· 部署 · 建远程资源 · 改共享云配置 · 数据库迁移 · 花钱 | **用户点头** |

三条配套:

1. **授权幂等**:同一项目内同类动作**一次点头长期有效**,不逐次追问。逐次问不是更安全——它把幂等的授权当成非幂等,**噪音会淹掉真正该问的那一次**(狼来了)。只有跨到没点过头的**新类别**(第一次 deploy、第一次建云资源、第一次花钱)才重新问。
...
2. **机械闸(pre-push 侧,2026-09-26 落地)**:`agent-on worktree hooks install` 装的 shared pre-push 多一条检查(`cli/src/prepush.rs`,<!-- src: cli/src/prepush.rs#local_base_merges -->):待推提交里有**非首 parent 已在 `origin/<default>` 上、committer ≠ GitHub** 的 merge commit,推向远端已有、非默认、且 `gh` 查得到**同仓、目标是默认分支**的开着的 PR 的分支 → 拦(别人 fork 里同名分支的 PR、进别的 base 的 PR 都不算:update-branch 合的是 PR 自己的 base)。**有意留的出口**:① 这笔 merge 有冲突就放行(`git merge-tree --write-tree` 判)——服务端 update-branch 解不了冲突(GitHub 回 422),本地 merge 是唯一出路,连它也拦就是案 40 的死锁;② git 太老(< 2.38)判不了冲突,不拦,打一行提示;③ 分支没有这样的 PR 就放行——没有服务端操作可撞。`gh` 查不出(没装、没登录、超时 10 秒)照拦,文案给查 PR 号的命令。**诚实边界**:只在装了 worktree hooks 的仓生效;PreToolUse 不判这条(它拿不到 git 给 pre-push 的待推范围);`rebase` 后强推不产生 merge commit,这道闸看不见,仍只靠第 1 条约束。
3. **出口面**:拦截文案按执行顺序给工单——撤掉本地 merge(被推的就是当前分支给 `git reset --keep <merge 前>`,不是就给带旧值校验的 `git update-ref`,别让照做的人挪错分支)、先推自己的提交、完整的 `gh api -X PUT repos/<owner>/<repo>/pulls/<N>/update-branch`、再把服务端那笔 merge 拉回来(§三½.5 4A:报错文案即工单)。逃生口沿用既有 `--no-verify`,**不新增逃生口**。

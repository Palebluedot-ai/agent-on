...
2. **机械闸(pre-push 侧,2026-09-26 落地)**:`agent-on worktree hooks install` 装的 shared pre-push 多一条检查(`cli/src/prepush.rs`,<!-- src: cli/src/prepush.rs#local_base_merges -->):待推提交里有**非首 parent 已在 `origin/<default>` 上、committer ≠ GitHub** 的 merge commit,推向远端已有、非默认、且 `gh` 查得到开着 PR 的分支 → 拦。**有意留的两个出口**:① 这笔 merge 有冲突就放行(`git merge-tree` 判)——服务端 update-branch 解不了冲突(GitHub 回 422),本地 merge 是唯一出路,连它也拦就是案 40 的死锁;② 分支没有开着的 PR 就放行——没有服务端操作可撞。`gh` 查不出(没装、没登录、超时 10 秒)照拦,文案给查 PR 号的命令。**诚实边界**:只在装了 worktree hooks 的仓生效;PreToolUse 不判这条(它拿不到 git 给 pre-push 的待推范围);`rebase` 后强推不产生 merge commit,这道闸看不见,仍只靠第 1 条约束。
3. **出口面**:拦截文案按执行顺序给工单——`git reset --keep <merge 前>`、先推自己的提交、完整的 `gh api -X PUT repos/<owner>/<repo>/pulls/<N>/update-branch`、再 `git pull --ff-only`(§三½.5 4A:报错文案即工单)。逃生口沿用既有 `--no-verify`,**不新增逃生口**。
